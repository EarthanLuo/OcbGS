...
- [ ] Spearman gate: computed over intersection of shared `cell_ids` only (cells present in both steps)
...
- [ ] Multi-step fixed-point: stable demand over consecutive steps → `δ ≈ 0` across all cells
- [ ] Multi-step no-thrash: small random perturbation of demand → dead-band absorbs it → `δ` changes only for cells above threshold
